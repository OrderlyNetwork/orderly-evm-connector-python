...
def create_clients(debug: bool = True):
    (
        orderly_key,
        orderly_secret,
        orderly_account_id,
    ...
    client_public = Client(orderly_testnet=orderly_testnet, debug=debug)
    ...
    return (
        ...
        client_private,
        client_async_public,
        client_async_private,
    )


def run_sync_examples(client_public: Client, client_private: Client) -> None:
    """Run a couple of synchronous REST endpoints and keep additional samples handy."""

    logging.info("Requesting registration nonce (sync public API)")
    client_public.get_registration_nonce()

    logging.info("Requesting current holdings (sync private API)")
    ...
    #     brokerId="woofi_pro",
    #     chainId=421614,
    #     receiver="0x6CBe925762348413fc2cfDD7bC9A8D04CB8E249e",
    #     token="USDC",
    #     amount=3000000,
    #     withdrawNonce=1,
    #     userAddress="0x6CBe925762348413fc2cfDD7bC9A8D04CB8E249e",
    # )
...
async def run_async_examples(
    ...
    logging.info("Requesting registration nonce (async public API)")
    await client_async_public.get_registration_nonce()

    logging.info("Requesting current holdings (async private API)")
    await client_async_private.get_current_holdings(True)
...
def main() -> None:
    ...
    run_sync_examples(client_public, client_private)
    asyncio.run(run_async_examples(client_async_public, client_async_private))
